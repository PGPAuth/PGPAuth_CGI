...
void PGPAuth(std::string data)
{
	...
	{
		cout << "Invalid signature! " << result->signatures[0].summary;
		return;
	}
	...
	string plainData(buffer);

	string request = plainData.substr(0, plainData.find_first_of(":"));

	if(request != "open" && request != "close")
		throw exception();

	string tstring = plainData.substr(plainData.find_first_of(":") + 1);
